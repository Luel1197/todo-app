...
import com.study.todoapp.dto.LoginRequestDto;
...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest req, HttpServletResponse res) throws AuthenticationException {
        ...
        if("application/json".equals(req.getContentType())){
            try {
                // 요청 받은 json -> 객체로 변환
                LoginRequestDto loginRequestDto = objectMapper.readValue(req.getInputStream(), LoginRequestDto.class);
                // 요청 받은 값 확인
                ...
                if(optionalUser.isPresent()){
                    ...
                    if (user.getPassword().equals(loginRequestDto.getPassword())) {
                        res.getWriter().write("비밀번호가 틀렸습니다.");
                    }
                } else {
                    res.setCharacterEncoding("UTF-8");
                    res.getWriter().write("해당 아이디로 가입한 계정이 없습니다.");
                }
